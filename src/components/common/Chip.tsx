...
const colorMap = {
  primary: {
    ...
    outline: "border border-indigo-600 text-indigo-600 dark:border-indigo-400 dark:text-indigo-400",
  },
  secondary: {
    solid: "bg-gray-600 text-white dark:bg-gray-500",
    outline: "border border-gray-600 text-gray-600 dark:border-gray-200 dark:text-gray-200",
  },
  danger: {
    solid: "bg-red-600 text-white  dark:bg-red-500",
    outline: "border border-red-600 text-red-600 dark:border-red-400 dark:text-red-400",
  },
  pink: {
    solid: "bg-pink-100 text-pink-700 dark:bg-pink-500 dark:text-pink-50",
    outline: "border border-pink-600 text-pink-600 dark:border-pink-400 dark:text-pink-400",
  },
} as const;
...
const Chip = ({ variant = "solid", color = "primary", size = "sm", className, icon, children }: ChipProps) => {
  const base = clsx("flex items-center gap-2", sizeMap[size], colorMap[color][variant], className);
